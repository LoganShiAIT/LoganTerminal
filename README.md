...
Phase 1 + 2 + 3 core all in. Not yet polished for daily use.
...
- Left FileTree that follows the shell's cwd via OSC 7 (an auto-generated zsh hook is injected via `ZDOTDIR`, chaining the user's own `~/.zshrc` first).
...
- Warm dark theme with a subtle Claude-orange grid pattern; xterm cursor and accents in Claude coral (`#d97757`).
...
| 4 | Polish — multi-tab (**done**), lightweight tab-layout restore (**done**), settings, themes |
...
Unit tests: `cd src-tauri && cargo test` — 28 tests covering the OSC parser, notification format matcher, screenshot filename recognizer, default start directory, Windows agent-name normalization, and clipboard image-file cleanup.
...
src/                   # React app
  ...
  stores/                # zustand (ptyStore = tabs, settingsStore = font size)
  lib/                   # shell-escape helper
